...
router.post('/', requireRole('admin'), (req, res) => {
  ...
  if (!['admin', 'teacher', 'student'].includes(role)) return res.status(400).json({ error: 'Некорректная роль' });
...
router.put('/:id', (req, res) => {
  ...
  if (isAdmin) {
    patch.role = (role && ['admin', 'teacher', 'student'].includes(role)) ? role : cur.role;
    if (login !== undefined) {
